    ...
    <div class="widget-content widget-content-area mb-3">
        ...
        <div class="widget-content">
            ...
            <div id="editPost" style="display:none">
                <form action="{{ route('data-kelas.forum.update', [$kelas->id, $post->id]) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method("put")
        ...
        <div class="col-md-12">
            <div class="d-flex flex-column comment-section">
                    ...
                    <div id="comment{{ $comment->id }}">
                        ...
                        <div class="mt-2">
                            ...
                            <div id="editComment{{ $comment->id }}" style="display:none">
                                <form action="{{ route('data-kelas.forum.comment.update', [$kelas->id, $post->id, $comment->id]) }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    @method("put")
                ...
                <div class="bg-light p-2" id="comment">
                    ...
                    <form action="{{ route('data-kelas.forum.comment.store', [$kelas->id, $post->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mt-2 text-right">
                            ...
                            <a href="{{ route('data-kelas.forum.index', $kelas->id) }}" class="btn btn-sm btn-dark-light ml-1 shadow-none">Kembali</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        function hapusPostingan() {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type:'DELETE',
                        url:'{{route("data-kelas.forum.destroy", [$kelas->id, $post->id])}}',
                        data:{
                            "_token": "{{ csrf_token() }}",
                        },
                        success:function(data) {
                            if (data.success){
                                Swal.fire(
                                    ...
                                    window.location.href = "{{ route('data-kelas.forum.index', $kelas->id)}}";
                                });
                            }
                        }
                    });
        ...
        function hapusKomentar(e) {  
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type:'DELETE',
                        url:'{{url("/admin/data-kelas/$kelas->id/forum/$post->id/comment")}}/' +id,
                        data:{
                            "_token": "{{ csrf_token() }}",
                        },
                        success:function(data) {
